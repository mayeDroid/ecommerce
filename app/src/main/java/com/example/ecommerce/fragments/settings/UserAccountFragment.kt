...
import com.example.ecommerce.viewmodel.UserAccountViewModel
...
class UserAccountFragment: Fragment() {
    ...
    private lateinit var imageActivityResultLauncher: ActivityResultLauncher<Intent> // to launch gallery
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        imageActivityResultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()){
            ...
            // to show the image in our image view
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launchWhenStarted {
            viewModel.updateOrEditInfo.collectLatest {
                when (it){
                    is Resource.Loading ->{
        ...
        binding.buttonSave.setOnClickListener {
            //create a user object then  and call update user from the view model
            binding.apply {
                val firstName = edFirstName.text.toString().trim()
                val lastName = edLastName.text.toString().trim()
                val email = edEmail.text.toString().trim()
                val user = User(firstName, lastName, email)
                viewModel.updateOrEditUserInfo(user,imageUri)
            }
        }

        binding.imageEdit.setOnClickListener {
            // click the button to launch gallery
    ...
    private fun showUserInformation(data: User) {
        binding.apply {
            ...
            edEmail.setText(data.firstName)
